...
class ECMMinerUnitTest {
    ...
    public void findTopInstrumentCountMusicianInMultiExact(int number) {
        ...
        for (int i = 0; i < number; i++){
            assertEquals("Keith Jarrett" + (number - i), musicians.get(i).getName());
        }
...
;       musicianList.get(4).setAlbums(new HashSet<Album>(Arrays.asList(album7))); //C = 1
    ...
    public void kTooLargeSocial(){
        HashSet<Musician> musicians = new HashSet<>();
        for (int i = 0; i < 5; i++){
            musicians.add(new Musician(""+i));
        }
        assertThrows(IllegalArgumentException.class, () -> ecmMiner.mostSocialMusicians(6));
    }
}